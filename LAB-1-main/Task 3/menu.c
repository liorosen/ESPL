...
char my_get(char c) {
    if (buffer[buf_pos] != '\0') {
        return buffer[buf_pos++];  // Return the current character and increment position
    } else {
        return '\0';  // Return null character if end of buffer is reached
    }
}

void reset_input() {
    fgets(buffer, sizeof(buffer), stdin); // Read a whole line when option 0 is selected
...
int main() {
    ...
    while (1) {
        ...
        if ( options < 0 || menu[options].name == NULL || options >= (sizeof(menu) / sizeof(menu[0]) - 1) || !menu[options].name) {
            ...
            //fgetc(stdin); /* Clear the newline character after the number input*/
