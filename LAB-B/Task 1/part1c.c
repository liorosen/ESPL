...
void scanBuffer(virus *v, char *buffer, unsigned int size){
    for (unsigned int i = 0; i < size - v->SigSize + 1; i++) {
        if (memcmp(buffer + i, v->sig, v->SigSize) == 0) {       // Taking examples from here : https://www.tutorialspoint.com/c_standard_library/c_function_memcmp.htm
...
void detect_virus_helper(char *buffer, unsigned int size) {
    link *current = virus_list;
    while (current != NULL) {
        virus *v = current->vir;
        scanBuffer(v, buffer, size);
        current = current->nextVirus;
    }
}

void fileBufferOpener(){
    
    FILE *file = fopen(suspectFileName, "rb");
    if (!file) {
    ...
    detect_virus_helper(buffer, size);
