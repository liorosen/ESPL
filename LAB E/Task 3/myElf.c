...
Elf32_Shdr* find_symbol_table_section(Elf32_Ehdr* header, Elf32_Shdr* section_headers) {
    // Find the symbol table section
    for (int i = 0; i < header->e_shnum; i++) { // The ELF header contains the number of section headers (e_shnum)
        // SHT_SYMTAB is a constant that identifies the symbol table section 
        if (section_headers[i].sh_type == SHT_SYMTAB) { 
            // If it is, return this section header
            return &section_headers[i];
        }
    }
    printf("No symbol table found.\n");
    return NULL; // Return NULL if no symbol table is found
}
...
void print_symbols_for_file(int fd, state* s) {
    ...
    // Find the symbol table section using the new helper function
    Elf32_Shdr* symtab_section = find_symbol_table_section(header, section_headers);
    if (symtab_section == NULL) {
        return; // Exit if no symbol table is found
    }
...
void handle_mergeable_sections(const char* section_name, state* s, int i, size_t* offset, Elf32_Shdr* new_section_headers, FILE* out_file, const char* merge_sections[], int merge_sections_length, Elf32_Shdr* section_headers1, Elf32_Shdr* section_headers2, bool* merged) {
...
void handle_non_mergeable_sections(state* s, int i, Elf32_Shdr* section_headers1, size_t* offset, FILE* out_file, Elf32_Shdr* new_section_headers, const char* section_name) {
    fwrite((char*)s->map_start1 + section_headers1[i].sh_offset, 1, section_headers1[i].sh_size, out_file);
    new_section_headers[i].sh_offset = *offset;
    *offset += section_headers1[i].sh_size;
...
void sections_Merging(state* s, Elf32_Ehdr* header1, const char* strtab1, Elf32_Shdr* section_headers1, Elf32_Shdr* section_headers2, size_t* offset, Elf32_Shdr* new_section_headers, FILE* out_file, const char* merge_sections[], int merge_sections_length) {
    for (int i = 0; i < header1->e_shnum; i++) {
        ...
        bool merged = false;
        handle_mergeable_sections(section_name, s, i, offset, new_section_headers, out_file, merge_sections, merge_sections_length, section_headers1, section_headers2, &merged);

        //Handle non-mergeable sections that are not specifically handled above
        if (!merged) {
            handle_non_mergeable_sections(s, i, section_headers1, offset, out_file, new_section_headers, section_name);
        }
    }
    printf("Finished sections_Merging\n");
}

void initialize_merge(state* s, Elf32_Shdr** symtab_section1, Elf32_Shdr** symtab_section2, Elf32_Shdr** section_headers1, Elf32_Shdr** section_headers2, const char** strtab1, const char** strtab2) {
    if (!check_merge_helper(s, symtab_section1, symtab_section2, section_headers1, section_headers2, strtab1, strtab2)) {
        perror("Error initializing merge");
        exit(1);
    }
}
...
Elf32_Shdr* allocate_new_section_headers(Elf32_Ehdr* header1) {
...
void write_new_section_headers(FILE* out_file, Elf32_Ehdr* header1, Elf32_Shdr* new_section_headers) {
    Elf32_Off shoff = ftell(out_file);
    fwrite(new_section_headers, 1, header1->e_shnum * sizeof(Elf32_Shdr), out_file);
    header1->e_shoff = shoff;
}

void write_new_section_headers_and_cleanup(FILE* out_file, Elf32_Ehdr* header1, Elf32_Shdr* new_section_headers) {
...
void merge_elf_files(state* s) {
    ...
    initialize_merge(s, &symtab_section1, &symtab_section2, &section_headers1, &section_headers2, &strtab1, &strtab2);

    FILE* out_file = open_output_file();
    Elf32_Ehdr* header1 = (Elf32_Ehdr*) s->map_start1;
    fwrite(header1, 1, sizeof(Elf32_Ehdr), out_file);

    Elf32_Shdr* new_section_headers = allocate_new_section_headers(header1);
    memcpy(new_section_headers, section_headers1, header1->e_shnum * sizeof(Elf32_Shdr));
    ...
    write_new_section_headers(out_file, header1, new_section_headers);
    ...
    write_new_section_headers_and_cleanup(out_file, header1, new_section_headers);
